...
import { buildPreheat, shouldPreheat, buildAlignment, isBoundaryRequest } from "./preheat.js";
import { makeDumper } from "./dump.js";
...
// v0.4.7：重申消息统一经此出口 —— 否则"重申到底发出去没有"同样不可见
function reanchorNotice() {
  DUMPER.record("reanchor", REANCHOR_TEXT, { channel: "agent/pre-step|tools/post-execute" });
  const summary = "破甲框架重申";
  return noticeMessage(REANCHOR_TEXT, summary);
}
  ...
  let preheat = null;
    ...
    try {
      ...
      if (PREHEAT_ON && !s.preheatDone && (!turn || turn <= 1)) {
        ...
        if (PREHEAT_ALIGN && isBoundaryRequest(opening)) {
          align = buildAlignment(opening);
        }
      }
    ...
    return Promise.resolve(next()).then((decision) => {
      try {
        if (!PRESTEP_INJECT || (!anchor && !preheat && !align)) return decision;
        if (!decision || decision.kind !== "enter" || !Array.isArray(decision.messages)) return decision;
        if (align) {
          const cls = align.cls || "default";
          DUMPER.record("align", align.task + "\n" + align.result, {
            cls,
          ...
          out.splice(
            at,
            0,
            preheatMessage("user", align.task, "align-task", "口径对齐：目标与授权边界"),
            preheatMessage("assistant", align.result, "align-result", "口径对齐：已确认"),
          );
        ...
        if (preheat) {
          ...
          DUMPER.record(
            "preheat",
            extra
              .map((m) => (m && m.content && m.content[0] ? m.content[0].text : ""))
              .join("\n---\n"),
            { mode: PREHEAT_MODE, messages: extra.length, channel: "agent/pre-step" },
          );
          const out = [...decision.messages];
          let at = out.length;
          for (let i = out.length - 1; i >= 0; i--) {
        ...
        return {
          ...decision,
          messages: [...decision.messages, reanchorNotice()],
        };
      } catch {
        return decision;
      }
