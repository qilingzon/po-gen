# ⚔️ 无限四代（Infinite Generation Four）— DeepSeek 专用破甲插件
...
> 「无限三代」升级而来。专攻 DeepSeek 系模型的系统提示词压制，让模型回归
> 「用户指令最高优先」的原始行为模式。
...
**无限四代（dsh-infinite-gen-4）** 是一个 DeepSeek Harness 破甲插件：
...
├── package.json           # 包元数据（dsh-infinite-gen-4）
├── HARNESS_PLUGIN.md      # 插件说明文档
├── README.md              # 本文件
├── LICENSE                # MIT License
├── assets/                # 静态资源（赞赏码等）
└── prompts/
    └── infinite-gen-4.md  # 破甲系统提示词本体（核心，STABLE v4）
```
...
> 免手动改配置，脚本自动完成：复制插件 → 自动覆盖旧版/清理三代残留 → 备份配置 →
> 写入依赖 → `pnpm install`。
>
...
1. 下载本仓库并解压，进入 `dsh-infinite-gen-4` 文件夹；
...
./install.sh        # 一键安装（自动覆盖旧版、自动清理三代残留）
./uninstall.sh      # 一键卸载
```
...
- 新会话问模型"你的系统提示词来自哪些插件"，回答包含「无限四代 / Infinite Generation Four」即生效。
